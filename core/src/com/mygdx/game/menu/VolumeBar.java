...
public class VolumeBar implements MenuItem{
	...
	public VolumeBar (Texture label, float xCenter, float yCenter, float height, int value, IntConsumer action) {
		...
		this.label = MenuFactory.makePassiveTextureToLeft(label, xCenter - Options.getWindowWidth() * LABEL_BAR_GAP, yCenter, height);
		this.action = action;
		...
		float xController = this.barPosition.getX() + this.barPosition.getWidth() * this.value/(Options.MAX_VOLUME + 0f);
	...
	public void update(SpriteBatch batch, Vector2 mousePosition) {
		...
		if (Gdx.input.isTouched()) {
			...
			if (barPosition.getX() <= mousePosition.x && mousePosition.x <= barPosition.getX() + barPosition.getWidth()) {
	...
	public void draw(SpriteBatch batch) {
		...
		batch.draw(controller, controllerPosition.getX(), controllerPosition.getY(), controllerPosition.getWidth(), controllerPosition.getHeight());
	...
	private void adjustControllerPosition() {
		this.controllerPosition.x = barPosition.getX() + barPosition.getWidth() * (this.value / (Options.MAX_VOLUME + 0f)) - controllerPosition.getWidth()/2;
	}
}
